...
define(function(require, exports, module) {
    ...
    function _bindEvents() {
        ...
        _demoboBindEvents.call(this);
    }

    function _demoboBindEvents() {
        this._eventInput.on('end', function(event){
            if (event.position>55 && this._onEdge) {
                this._eventOutput.emit('pullToRefresh');
            }
        }.bind(this));
        if (!this.id) return;
        this._eventInput.on('start', function(event){
            localStorage.setItem("DemoboStorage-scrollviewStart_"+this.id, JSON.stringify(event));
        }.bind(this));
        this._eventInput.on('update', function(event){
            event.normalSize = this._node.getSize()[this.options.direction];
            this.setParticleMass(this._node.getSize()[this.options.direction]/event.normalSize);
            localStorage.setItem("DemoboStorage-scrollviewMove_"+this.id, JSON.stringify(event));
        }.bind(this));
        this._eventInput.on('end', function(event){
            localStorage.setItem("DemoboStorage-scrollviewEnd_"+this.id, JSON.stringify(event));
        }.bind(this));
        this._scroller.on('onEdge', function(data) {
            localStorage.setItem("DemoboStorage-scrollviewOnEdge_"+this.id, JSON.stringify(data));
        }.bind(this));
        this._scroller.on('offEdge', function() {
            localStorage.setItem("DemoboStorage-scrollviewOffEdge_"+this.id, Date.now());
        }.bind(this));
        this._particle.on('update', function(particle) {
            var p = {position: particle.position};
            localStorage.setItem("DemoboStorage-scrollviewPU_"+this.id, JSON.stringify(p));
        }.bind(this));
        this._particle.on('end', function() {
            localStorage.setItem("DemoboStorage-scrollviewPE_"+this.id, Date.now());
        }.bind(this));

        var self = this;
        window.addEventListener("storage", function(evt) {
            if (evt.key=="DemoboStorage-scrollviewStart_"+this.id) {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleStart.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewMove_"+this.id) {
                var scrollEvt = JSON.parse(evt.newValue);
                var percentage = scrollEvt.delta/scrollEvt.normalSize;
                this.setParticleMass(this._node.getSize()[this.options.direction]/scrollEvt.normalSize);
                scrollEvt.delta = percentage*this._node.getSize()[this.options.direction];
                _handleMove.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewEnd_"+this.id) {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleEnd.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewOnEdge_"+this.id) {
                var data = JSON.parse(evt.newValue);
                self._edgeSpringPosition = data.position;
                _handleEdge.call(self, self._scroller.onEdge());
                self._eventOutput.emit('onEdge');
            } else if (evt.key=="DemoboStorage-scrollviewOffEdge_"+this.id) {
                self.sync.setOptions({scale: self.options.syncScale});
                self._onEdge = self._scroller.onEdge();
                self._eventOutput.emit('offEdge');
            } else if (evt.key=="DemoboStorage-scrollviewPU_"+this.id) {
                var particle = JSON.parse(evt.newValue);
                if (self._springState === SpringStates.NONE) _normalizeState.call(self);
                self._displacement = particle.position.x - self._totalShift;
            } else if (evt.key=="DemoboStorage-scrollviewPE_"+this.id) {
                if (!self.options.paginated || (self.options.paginated && self._springState !== SpringStates.NONE))
                    self._eventOutput.emit('settle');
            }
        }.bind(this));
    }
    ...
    Scrollview.prototype.setParticleMass = _.once(function setParticleMass(percentage){
        this._particle.setMass(percentage)
    });
